...
void quat_bench_lll_from_file(){
    ...
    for(int i = 0; i<LATTICE_DATA_NUMBER;i++){
        ...
        start = cpucycles();
        quat_lattice_lll(&red, &lat,&(alg.p),0);
        cycles = cpucycles()-start;
        fprintf(f,"%"PRIu64"\n", cycles);
        if(i%100==0){
            printf("%d lattices are lll reduced \n ", i);
        }
    }
...
int main(int argc, char* argv[]){
    ...
    for (int i = 0; i<NTESTS; i++){
        read_lattice_from_file(&lat,i);
        res = res | quat_test_lll_on_lattice(&lat,&alg);
    }
    quat_lattice_finalize(&lat);
    quat_alg_finalize(&alg);

    if (res){
        printf("Test lll_on_list failed\n");
    }
    else{
        printf("Test lll_on_list passed\n");
